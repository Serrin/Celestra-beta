...
const lte = (value1, value2) => lt(value1, value2) || eq(value1, value2);
...
const getUrlVars = (str = location.search) => Object.fromEntries([...new URLSearchParams(str).entries()]);
...
function timestampID(size = 21, alphabet = "23456789CFGHJMPQRVWXcfghjmpqvwx") {
    if (!Number.isSafeInteger(size)
        || size < 11
        || alphabet.length > 255) {
        throw new RangeError("[timestampID] Size should be an integer in between 11 and 255.");
    }
    if (typeof alphabet !== "string"
        || !alphabet.length
        || alphabet.length > 255) {
...
const partition = ([...array], callback) => [array.filter(callback), array.filter((value, index, array) => !(callback(value, index, array)))];
