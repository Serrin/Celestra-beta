...
function domCreate(elType, properties, innerHTML) {
...
const domGetCSS = (el, property) => (property ? globalThis.getComputedStyle(el, null)[property] :
    globalThis.getComputedStyle(el, null));
function domSetCSS(el, property, value) {
...
function domFadeIn(el, duration, display) {
    let s = el.style;
...
function domFadeOut(el, duration) {
    let style = el.style;
    let step = 25 / (duration || 500);
...
function domFadeToggle(el, duration, display = "") {
...
const domHide = (el) => el.style.display = "none";
const domShow = (el, display = "") => el.style.display = display;
function domToggle(el, display = "") {
    if (globalThis.getComputedStyle(el, null).display === "none") {
...
const domIsHidden = (el) => (globalThis.getComputedStyle(el, null).display === "none");
...
const domScrollToElement = (el, top = true) => el.scrollIntoView(top);
