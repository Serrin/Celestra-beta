...
function domCreate (
  ...
  if (arguments.length === 1 && typeof elementType === "object") {
    let obj = elementType;
    elementType = obj.elementType as string;
    properties = {};
    for (let key in obj) {
      /* @ts-ignore */
...
function setCookie (
  ...
  if (name !== null && typeof name === "object") {
    ...
    hours = settings.hours as number || 8760 as number;
    path = settings.path as string || "/" as string;
    domain = settings.domain as string;
    secure = settings.secure as boolean;
    SameSite = settings.SameSite as string || "Lax" as string;
    HttpOnly = settings.HttpOnly as boolean;
  }
  let expire = new Date();
 ...
 *
 * @param {any[]} array - The iterable to search through.
 * @param {Function} callback - The function to test each element.
 * @returns {any} The first element that satisfies the testing function, or undefined if none do.
 */
const find = ([...array]: any[], callback: Function): unknown =>
