...
function unique(it,resolver){if(resolver==null){return[...new Set(it)];}if(typeof resolver==="string"){return Array.from(it).reduce(function(acc,el){if(acc.every((e)=>e[resolver]!==el[resolver])){acc.push(el);}return acc;},[]).values();}if(typeof resolver==="function"){let cache=new Map();for(let item of it){let key=resolver(item);if(!cache.has(key)){cache.set(key,item);}}return cache.values();}}
